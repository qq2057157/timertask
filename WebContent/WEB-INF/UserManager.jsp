...
$(function(){
    ...
    onPageClicked: function (e, originalEvent, type, page) {  
         ...
         location.href = '/timer/user/main?currentPage='+page;
         //console.log(e);
...
<div class="row garybgcolor">
	...
	<div id="data" class="col-xs-10 table-responsive">
		<%@ include  file='../views/UsersView.jsp'%>
	</div>
</div>
</div>
...
$(document).ready(function(){
		...
		if(isEmpty(username) || 
			...
			$.ajax({
	    		...
	    		success:function(data){
	    			if(data.code ==99999){
	    				alertModalMessage("添加用户成功");
	    				$(".closeMessage").addClass("refresh-btn");
	    			} else if(data.code == 505) {
	    				alertModalMessage("服务器发生异常，添加用户失败");
	    			} else {
	    				alertModalMessage("添加用户失败");
	    			}
	    		}
	    		});
			// ajax end;
		...
		$.ajax({
    		...
    		success:function(data){
    			if(data.code==99999){
    				...
    				mythis.html("解禁");
    				mythis.addClass("enableUser");
    				mythis.removeClass("disableUser");
		...
		$.ajax({
    		...
    		success:function(data){
    			if(data.code==99999){
    				...
    				//mythis.parents('td').html('<a class="taskStop" href="javascript:void(0);">终止</a>');
    				mythis.html("封禁");
    				mythis.addClass("disableUser");
    				mythis.removeClass("enableUser");
    			} else if(data.code==505){
	...
	$(document).on("click",".setAdminRole",function(){
		//alert($(this).parents('td').prev().find('input.status').val());
		var mythis = $(this);
		//alert($(this).parents('td').find('input.id').val());
		$.ajax({
    		url:"/timer/user/setAdmin",
    		data:{
    			id:$(this).parents('td').find('input.id').val(),
    			organizationCode:$(this).parents('td').find('input.organizationCode').val()
    		},
    		type:'get',
    		dataType:'json',
    		success:function(data){
    			if(data.code==99999){
    				alertModalMessage("设置成功");
    				$(".closeMessage").addClass("refresh-btn");
     			} else if(data.code==505){
     				alertModalMessage("设置失败,服务器异常");		
    			} else {
    				alertModalMessage("设置失败");
    			}
    		}
    		});
		//ajax end;
